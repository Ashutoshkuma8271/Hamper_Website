...
import { ChevronLeft, ChevronRight, Plus, Sparkles, Loader2, Heart, ArrowRight, Star, Eye, Zap, ShoppingBag } from 'lucide-react';
...
function ProductCard({
  ...
  return (
    <article
      className="group relative flex flex-col justify-between rounded-3xl bg-cream-50 dark:bg-gray-800 ring-1 ring-cream-200 dark:ring-gray-700 overflow-hidden transition-all duration-500 hover:ring-gold-400/50 hover:-translate-y-1.5 hover:shadow-[0_30px_60px_-30px_rgba(87,34,44,0.45)]"
      style={{ transitionDelay: `${delay}ms` }}
    >
      <div>
        <div className="relative aspect-[4/3] overflow-hidden bg-cream-100 dark:bg-gray-900">
          <img
            src={product.image}
            alt={product.name}
            loading="lazy"
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1 z-10">
            {product.tag && (
              <span className="inline-flex items-center gap-1 rounded-full bg-wine-700/90 backdrop-blur px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-cream-50 shadow-sm">
                <Sparkles className="h-3 w-3 text-gold-400" />
                {product.tag}
              </span>
            )}
            {discountPercent > 0 && (
              <span className="inline-flex items-center rounded-full bg-gold-500 text-wine-900 font-bold px-2.5 py-0.5 text-[10px] uppercase shadow-sm">
                {discountPercent}% OFF
              </span>
            )}
          </div>

          <span className="absolute top-3 right-3 rounded-full bg-cream-50/90 backdrop-blur px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-wine-600 dark:bg-gray-800 dark:text-gold-300">
            {product.category}
          </span>
          ...
          <button
            ...
            className={`absolute left-3 bottom-3 grid h-9 w-9 place-items-center rounded-full bg-cream-50/95 backdrop-blur transition-transform hover:scale-110 dark:bg-gray-800/95 ${
              saved ? 'text-wine-600' : 'text-ink-700 hover:text-wine-600'
            }`}
          >
            <Heart className={`h-4 w-4 ${saved ? 'fill-wine-600 text-wine-600' : ''}`} />
          </button>
          ...
          <div className="absolute inset-x-3 bottom-3 opacity-0 transition-opacity duration-300 group-hover:opacity-100 flex justify-end">
            <button
              ...
              className="inline-flex items-center justify-center gap-1 rounded-full bg-white/95 text-wine-800 px-3 py-1.5 text-xs font-semibold shadow-lg backdrop-blur-sm hover:bg-wine-600 hover:text-white transition-colors"
            >
        ...
        <div className="flex flex-col p-5">
          <div className="flex items-center justify-between mb-1">
            ...
            <div className="flex items-center gap-1 text-xs font-bold text-ink-800 dark:text-gray-200">
              <Star className="h-3.5 w-3.5 fill-gold-500 text-gold-500" />
              <span>{(product as any).rating || '4.9'}</span>
              <span className="text-[10px] text-ink-700/40 font-normal">(124)</span>
            </div>
          </div>

          <h3
            onClick={() => onQuickView(product)}
            className="font-display font-semibold text-wine-700 text-lg leading-snug dark:text-white cursor-pointer hover:text-gold-600 transition-colors line-clamp-1"
          >
            {product.name}
          </h3>

          <p className="mt-1.5 text-xs text-ink-700/70 leading-relaxed dark:text-gray-300 line-clamp-2">
            {product.description}
          </p>
        </div>
      </div>
      ...
      <div className="p-5 pt-0">
        <div className="flex items-baseline gap-2 mb-3 pt-3 border-t border-cream-200 dark:border-gray-700">
          <span className="font-display font-bold text-lg text-wine-800 dark:text-gold-400">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            onClick={() => add(product)}
            className="w-full inline-flex h-9 items-center justify-center gap-1 rounded-full border border-wine-600/30 bg-cream-50 dark:bg-gray-700 px-3 text-xs font-semibold text-wine-700 dark:text-cream-100 hover:bg-wine-600 hover:text-white transition-colors"
          >
            <ShoppingBag className="h-3.5 w-3.5" /> Add
          </button>
          <button
            onClick={handleBuyNow}
            className="w-full inline-flex h-9 items-center justify-center gap-1 rounded-full bg-wine-600 px-3 text-xs font-semibold text-cream-50 hover:bg-wine-700 transition-colors shadow-sm"
          >
